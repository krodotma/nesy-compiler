...
export class FileBus extends Bus {
  private fileConfig: Required<FileBusConfig>;
   ...
   */
  private getEventFilePath(topic?: string): string {
  ...
  private async maybeRotate(): Promise<void> {
    if (this.currentFileSize < this.fileConfig.maxFileSize) return;
  ...
  async readEvents(
    ...
    let events: BusEvent[] = lines.map((line) => JSON.parse(line));
 ...
 */
export function createFileBus(config: FileBusConfig): FileBus {
