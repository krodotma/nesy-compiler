...
const BUS_FAMILY = new Set<ViewId>(['bus', 'events', 'agents', 'requests']);
...
export const BicameralNav = component$<BicameralNavProps>(({ activeView, onSelect$ }) => {
  ...
  return (
    <nav class="nav-3d-inline border-b border-border px-6 py-4 flex-shrink-0 overflow-x-auto">
      <div class="flex flex-col gap-3">
        <div class="flex flex-wrap items-center gap-2">
          ...
          {CORE_VIEWS.map((view) => (
            <button
              key={view.id}
              class={buttonClass(activeView === view.id)}
        ...
        <div class="grid grid-cols-1 gap-3 lg:grid-cols-3">
          ...
          <section class="space-y-2">
            <div class="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
              <span class="h-2 w-2 rounded-full bg-cyan-400/70" />
              MLOps
            </div>
            <div class="flex flex-wrap gap-2">
