...
export class PipelineOrchestrator {
    ...
    async start(
        ...
        try {
            ...
            this.ringBuffer = new AudioRingBuffer(80000); // 5 sec buffer
            this.mixer = new AudioMixer(24000); // Vocos is 24kHz

            // 4. Create Worklet Node
            this.workletNode = new AudioWorkletNode(this.audioContext, 'auralux-processor');
            this.workletNode.port.postMessage({
                type: 'INIT',
                sharedBuffer: this.ringBuffer.sharedBuffer
            });
