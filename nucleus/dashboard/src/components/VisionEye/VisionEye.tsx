...
export const VisionEye = component$(() => {
    ...
    const sendSnapshot = $(async () => {
        ...
        try {
            ...
            const res = await fetch('http://localhost:8090/v1/eyes/ingest', {
                method: 'POST',
    ...
    return (
        <div class="flex items-center gap-2 px-3 py-1 bg-slate-800/50 rounded-full border border-slate-700/50 backdrop-blur-sm">
            ...
            <div class="relative">
                <div class={`w-2 h-2 rounded-full ${active.value ? 'bg-green-500' : 'bg-slate-500'}`} />
                {active.value && (
            ...
            {!active.value ? (
                ...
                <div class="flex items-center gap-2">
                    {/* ANALYZING indicator */}
                    <span class="text-[10px] font-bold text-green-400 animate-pulse tracking-wider">
                        ANALYZING
                    </span>
