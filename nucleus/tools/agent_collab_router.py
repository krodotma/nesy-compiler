...
DEFAULT_RESPONSE_TOPIC = "agent.collab.response"
DEFAULT_RESPONSE_TOPICS = [
    "agent.collab.request",
    "agent.collab.response",
    "agent.collab",
    "agent.collab.*",
    "cagent.collab.response",
    "cagent.collab.*",
]
OUTPUT_LIMIT = 12
...
def build_prompt(event: dict) -> str:
    ...
    target = data.get("target") or data.get("provider") or data.get("providers") or data.get("to")
...
def _response_patterns(response_topic: str | None) -> list[str]:
    extra = [response_topic] if response_topic else []
    return _dedupe([*DEFAULT_RESPONSE_TOPICS, *extra])
...
def emit_collab_response(
    *,
    ...
    topics = _dedupe([t for t in [response_topic, origin_topic] if t])
    for topic in topics:
        emit_bus(
            bus_dir,
            topic=topic,
...
def process_once(
    *,
    ...
    response_patterns = _response_patterns(response_topic)
    ...
    for e in events:
        ...
        if e.get("topic") == "dialogos.cell.end" and e.get("kind") == "response":
            end_by_req[req_id] = e
        if e.get("kind") == "response" and _topic_matches(str(e.get("topic") or ""), response_patterns):
            ...
            if topic == "dialogos.cell.end" and kind == "response":
                ...
                if req_id in forwarded and req_id not in responded:
                    ...
                    if origin_topic:
                        ...
                        responded.add(req_id)
            if kind == "response" and _topic_matches(topic, response_patterns):
                responded.add(req_id)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agent_collab_router.py",
        description="Route agent.collab (agent.collab.* / cagent.collab.*) requests into dialogos.submit and emit responses.",
    )
    ...
    parser.add_argument("--response-topic", default=os.environ.get("COLLAB_RESPONSE_TOPIC", DEFAULT_RESPONSE_TOPIC))
...
def main(argv: list[str]) -> int:
    ...
    response_topic = str(args.response_topic or "").strip()
    if response_topic.lower() in {"", "none", "origin"}:
        response_topic = None
