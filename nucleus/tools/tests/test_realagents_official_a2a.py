...
async def _wait_http_ready(url: str, *, timeout_s: float = 10.0) -> None:
    import httpx
...
class TestRealAgentsOfficialA2A(unittest.TestCase):
    def test_a2a_official_server_roundtrip(self) -> None:
        try:
        ...
        server_script = repo_root / "nucleus" / "tools" / "a2a_official_server.py"
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            env = {**os.environ, "PLURIBUS_BUS_DIR": str(bus_dir), "PLURIBUS_ACTOR": "test-a2a", "PLURIBUS_ALLOW_MOCK": "1"}
            proc = subprocess.Popen(
                [python, str(server_script), "--host", "127.0.0.1", "--port", str(port), "--provider", "mock"],
