...
try:
    ...
    from .core.bus_consumer_v3 import BusConsumer, ConsumerMetrics
except Exception:
    from agent_bus import resolve_bus_paths, emit_event, default_actor
    from qa_ir import normalize_event, QAIrStore
    from graphiti_bridge import GraphitiService, extract_facts_from_bus_event
...
class DialogosIndexer:
    ...
    def __init__(
        ...
        *,
        ...
        spine_snapshot_path: Optional[Path] = None,
        spine_export: bool = False,
        spine_registry_root: Optional[Path] = None,
    ):
        self.root = root
        ...
        self.spine_snapshot_path = spine_snapshot_path
        self.spine_export = spine_export
        self.spine_registry_root = spine_registry_root
    ...
    def _init_chroma(self) -> None:
        ...
        DE-PRIMITIVE MIGRATION (2026-01-16):
        Falls back to FalkorDB AdvancedService if ChromaDB not available.
        FalkorDB is the canonical storage layer.
        See: nucleus/docs/falkordb_integration_guide.md
        """
        ...
        try:
            ...
            # ChromaDB not available - try FalkorDB fallback
            try:
                from falkordb_advanced import AdvancedService
                import uuid
                import json as _json

                class FalkorDBMemoryShim:
                    """Shim that wraps FalkorDB to provide ChromaMemory-like interface."""
                    def __init__(self):
                        self._service = AdvancedService()
                        self._service.connect()

                    def store(self, text: str, metadata: dict = None):
                        """Store memory using Cypher query."""
                        cypher = """
                            CREATE (m:dialogos_memory {
                                id: $id,
                                content: $content,
                                agent_id: $agent_id,
                                memory_type: 'dialogos_embedding',
                                metadata: $metadata,
                                created_at: $created_at
                            })
                            RETURN m.id
                        """
                        self._service._graph.query(cypher, {
                            "id": str(uuid.uuid4()),
                            "content": text[:2000] if text else "",
                            "agent_id": metadata.get("agent", "dialogos") if metadata else "dialogos",
                            "metadata": _json.dumps(metadata or {}),
                            "created_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                        })

                    def query(self, query: str, top_k: int = 5):
                        """Query memories using Cypher CONTAINS search."""
                        cypher = """
                            MATCH (m:dialogos_memory)
                            WHERE m.content CONTAINS $query
                            RETURN m.id, m.content, m.metadata
                            LIMIT $limit
                        """
                        result = self._service._graph.query(cypher, {"query": query, "limit": top_k})
                        return [
                            {"id": row[0], "content": row[1], "metadata": row[2]}
                            for row in (result.result_set or [])
                        ]

                self.chroma = FalkorDBMemoryShim()
                self._emit_bus(
                    "dialogos.indexer.info",
                    {"component": "memory", "backend": "falkordb", "note": "Using FalkorDB fallback"},
                    level="info",
                )
            except Exception:
                self.chroma = None
    ...
    def _resolve_spine_snapshot_path(self) -> Path:
        """Resolve the spine registry snapshot path."""
        if self.spine_snapshot_path:
            return self.spine_snapshot_path
        env_path = os.environ.get("PLURIBUS_SPINE_REGISTRY_SNAPSHOT")
        if env_path:
            return Path(env_path)
        return self.root / ".pluribus" / "spine" / "spine_registry_snapshot.ndjson"

    def _resolve_spine_registry_root(self) -> Path:
        """Resolve the spine registry root directory."""
        if self.spine_registry_root:
            return self.spine_registry_root
        env_root = os.environ.get("PLURIBUS_SPINE_REGISTRY_ROOT")
        if env_root:
            return Path(env_root)
        return self.root / "spine" / "registry"

    def _export_spine_snapshot(self, out_path: Path) -> dict:
        """Export spine registry snapshot to an NDJSON file."""
        tool_path = self.root / "spine" / "tools" / "export_registry_snapshot.py"
        registry_root = self._resolve_spine_registry_root()
        if not tool_path.exists():
            return {"ok": False, "error": "export_tool_missing", "tool": str(tool_path)}
        if not registry_root.exists():
            return {"ok": False, "error": "registry_root_missing", "registry_root": str(registry_root)}
    ...
    def _summarize_spine_snapshot(self, path: Path) -> dict:
        """Summarize snapshot counts by kind."""
        total = 0
        kinds: dict[str, int] = {}
        bad_lines = 0
        ...
        return {"total": total, "kinds": kinds, "bad_lines": bad_lines}

    def _maybe_emit_spine_snapshot(self) -> Optional[dict]:
        """Emit a spine snapshot metadata event if available."""
        snapshot_path = self._resolve_spine_snapshot_path()
        export_info: Optional[dict] = None

        if self.spine_export:
            export_info = self._export_spine_snapshot(snapshot_path)
            if not export_info.get("ok"):
                payload = {
                    "ok": False,
                    "error": "export_failed",
                    "export": export_info,
                ...
                self._emit_bus("dialogos.indexer.spine_snapshot", payload, level="warn")
                return payload

        if snapshot_path.exists():
            summary = self._summarize_spine_snapshot(snapshot_path)
            payload = {
                "ok": True,
                "path": str(snapshot_path),
                "summary": summary,
            ...
            self._emit_bus("dialogos.indexer.spine_snapshot", payload)
            return payload
    ...
    def _tail_trace_file(self) -> Iterator[dict]:
        """Tail the trace file for new records using BusConsumer v3.

        Supports both file backend (default) and WebSocket backend for real-time
        consumption via bus-bridge.
        """
        ensure_file(self.trace_path)

        # Determine state path for consumer
        state_path = self.state_path.parent / "consumer_state.json" if self.state_path else None

        # Select backend based on environment
        use_websocket = os.environ.get("DIALOGOS_INDEXER_USE_WS", "").lower() in ("1", "true", "yes")

        if use_websocket:
            ws_url = os.environ.get("BUS_WS_URL", "ws://localhost:9200")
            ws_kwargs = {
                "ws_url": ws_url,
                "state_path": state_path,
                "subscriptions": ["dialogos.*"],
                "consumer_group": "dialogos-indexer",
                "instance_id": f"indexer-{os.getpid()}",
            }
            try:
                consumer = BusConsumer.websocket(**ws_kwargs)
            except TypeError as exc:
                # Fallback for v2 BusConsumer without consumer_group/instance_id
                if "consumer_group" in str(exc) or "instance_id" in str(exc):
                    ws_kwargs.pop("consumer_group", None)
                    ws_kwargs.pop("instance_id", None)
                    consumer = BusConsumer.websocket(**ws_kwargs)
                else:
                    raise
            if hasattr(consumer, "connect"):
                consumer.connect()
            print(f"[dialogos-indexer] Using WebSocket backend: {ws_url}", file=sys.stderr)
        else:
            file_kwargs = {
                "bus_path": self.trace_path,
                "state_path": state_path,
                "consumer_group": "dialogos-indexer",
                "instance_id": f"indexer-{os.getpid()}",
            }
            try:
                consumer = BusConsumer(**file_kwargs)
            except TypeError as exc:
                if "consumer_group" in str(exc) or "instance_id" in str(exc):
                    file_kwargs.pop("consumer_group", None)
                    file_kwargs.pop("instance_id", None)
                    consumer = BusConsumer(**file_kwargs)
                else:
                    raise
            print(f"[dialogos-indexer] Using file backend: {self.trace_path}", file=sys.stderr)

        try:
            for event in consumer.consume(tail=True, poll_interval=self.poll_s):
                if not self._running:
                    break

                # Handle heartbeat (None events during tail)
                if event is None:
                ...
                # Periodically ack and emit metrics
                if self.stats.records_indexed % 100 == 0:
                    consumer.ack()
                    # Emit consumer metrics if v3 metrics available
                    if ConsumerMetrics is not None and hasattr(consumer, "emit_metrics"):
                        consumer.emit_metrics(topic="dialogos.consumer.metrics")
    ...
    def run_daemon(self) -> int:
        ...
        spine_snapshot = self._maybe_emit_spine_snapshot()
        self._emit_bus(
...
def cmd_daemon(args: argparse.Namespace) -> int:
    ...
    indexer = DialogosIndexer(
        ...
        spine_snapshot_path=Path(args.spine_snapshot).expanduser().resolve() if args.spine_snapshot else None,
        spine_export=args.spine_export,
        spine_registry_root=Path(args.spine_registry).expanduser().resolve() if args.spine_registry else None,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    daemon.add_argument(
        "--spine-snapshot",
        default=os.environ.get("PLURIBUS_SPINE_REGISTRY_SNAPSHOT"),
        help="Path to spine registry snapshot NDJSON (optional)",
    )
    daemon.add_argument(
        "--spine-export",
        action="store_true",
        help="Export spine registry snapshot on boot (requires /pluribus/spine)",
    )
    daemon.add_argument(
        "--spine-registry",
        default=os.environ.get("PLURIBUS_SPINE_REGISTRY_ROOT"),
        help="Override spine registry root for snapshot export",
    )
