...
def resolve_agent_bin(name: str, fallback: str) -> str:
    ...
    if env_val:
        return env_val
    wrapper = Path(__file__).resolve().parent / f"bus-{name}"
...
KNOWN_AGENTS: dict[str, AgentConfig] = {
    "claude": AgentConfig(
        name="claude",
        path=resolve_agent_bin("claude", "/usr/local/bin/claude"),
        args_template=["--dangerously-skip-permissions", "-p"],
        mode="prompt",
    ),
    "codex": AgentConfig(
        name="codex",
        path=resolve_agent_bin("codex", "/usr/local/bin/codex"),
        args_template=["exec", "--full-auto", "--ask-for-approval", "never", "-s", "workspace-write"],
        mode="codex_exec",
    ),
    "gemini": AgentConfig(
        name="gemini",
        path=resolve_agent_bin("gemini", "/usr/bin/gemini"),
        args_template=["-p"],
        mode="prompt",
    ),
}
...
def check_agent_availability() -> None:
    ...
    for name, cfg in KNOWN_AGENTS.items():
        if os.path.isabs(cfg.path):
            cfg.available = os.path.isfile(cfg.path) and os.access(cfg.path, os.X_OK)
        else:
            cfg.available = shutil.which(cfg.path) is not None
...
class AgentCascadeDaemon:
    def __init__(
        ...
        self.events_inode: Optional[int] = None
    ...
    def process_new_events(self) -> None:
        ...
        if self.events_inode is None:
            self.events_inode = inode
        elif inode is not None and self.events_inode is not None and inode != self.events_inode:
            # Reset when the bus file is rotated/replaced.
            self.last_file_pos = 0
            self.events_inode = inode

        if st.st_size < self.last_file_pos:
            # Reset if the file was truncated.
            self.last_file_pos = 0
    ...
    def spawn_agent(self, task: PendingTask) -> None:
        ...
        if os.environ.get("PLURIBUS_CASCADE_BOOTSTRAP", "1") != "0":
            bootstrap = BOOTSTRAP_PROMPT.format(agent=task.target)
            prompt = f"{bootstrap}\n\n{prompt}"
    ...
    def run(self) -> None:
        ...
        if self.events_path.exists():
            st = self.events_path.stat()
            self.events_inode = getattr(st, "st_ino", None)
            self.last_file_pos = st.st_size
        else:
            self.events_inode = None
            self.last_file_pos = 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--max-concurrent", type=int, default=None, help="Max concurrent agent spawns")
...
def main(argv: list[str]) -> int:
    ...
    if max_concurrent is None:
        max_concurrent = int(os.environ.get("CASCADE_MAX_CONCURRENT", "1"))
