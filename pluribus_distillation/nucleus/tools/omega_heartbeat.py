...
# Pending pair TTL - expire stale requests after this duration (1 hour)
PENDING_PAIR_TTL_S = 3600.0
...
class OmegaState:
    ...
    def expire_stale_pairs(self, ttl_s: float = PENDING_PAIR_TTL_S) -> int:
        """
        Expire pending pairs older than TTL.

        Returns number of pairs expired.
        """
        now = now_ts()
        expired_keys = []

        for key, req_ts in self.pending_pairs.items():
            if (now - req_ts) > ttl_s:
                expired_keys.append(key)

        for key in expired_keys:
            del self.pending_pairs[key]
            # Also clean up request_timestamps if present
            req_id = key[1] if len(key) > 1 else None
            if req_id and req_id in self.request_timestamps:
...
class OmegaHeartbeat:
    ...
    def run_cycle(self) -> None:
        ...
        # Expire stale pending pairs (TTL-based cleanup)
        expired = self.state.expire_stale_pairs()
        if expired > 0:
            self._log(f"Expired {expired} stale pending pairs (TTL={PENDING_PAIR_TTL_S}s)")
