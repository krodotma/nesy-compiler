...
EPI_GAP_WARN_DEFAULT = 0.2
EPI_GAP_MIN_DOCS_DEFAULT = 50
MEM_WARN_PCT_DEFAULT = 85.0
MEM_CRIT_PCT_DEFAULT = 92.0
SWAP_WARN_PCT_DEFAULT = 20.0
SWAP_CRIT_PCT_DEFAULT = 40.0
PSI_SOME_WARN_DEFAULT = 0.1
PSI_FULL_WARN_DEFAULT = 0.05
PSI_FULL_CRIT_DEFAULT = 0.1
WAVE_WIDTH = 48
...
class OmegaHeartMonitor:
    def __init__(self, filter_name=None, bootstrap_tasks=True):
        ...
        self.emit_memory_pressure = os.environ.get("OHM_EMIT_MEMORY_PRESSURE", "1") != "0"
        self._last_emit_ts = 0.0
        self._last_alert_ts = 0.0
        self._last_art_tick_ts = 0.0
        self._last_alert_state = None
        self._last_memory_pressure_state = None
        self._last_memory_pressure_ts = 0.0
    ...
    def _status_payload(self) -> dict:
        ...
        return {
            ...
            "memory": {
                "epistemic_gap": self._epistemic_gap(),
                **self._memory_status(),
            },
            "omega": {
    ...
    def _emit_status(self, *, force: bool = False) -> None:
        ...
        if self.emit_memory_pressure:
            self._emit_memory_pressure(payload, now)
    ...
    def _emit_memory_pressure(self, payload: dict, now: float) -> None:
        ...
        if not state:
            return
        if state != self._last_memory_pressure_state or (now - self._last_memory_pressure_ts) > (self.emit_interval_s * 4):
            level = "info"
            if state == "elevated":
                level = "warn"
            elif state == "critical":
                level = "error"
            self._emit_bus_event(topic="ohm.memory.pressure", kind="metric", level=level, data=memory)
            self._last_memory_pressure_state = state
            self._last_memory_pressure_ts = now
    ...
    def _epistemic_gap(self) -> dict:
        """Compare deterministic (FTS) vs semantic (vector) coverage in RAG."""
        root = Path(os.environ.get("PLURIBUS_ROOT", str(REPO_ROOT))).resolve()
        db_path = root / ".pluribus" / "index" / "rag.sqlite3"
        result = {
            "status": "unknown",
            "db_path": str(db_path),
            "fts_entries": None,
        ...
        try:
            ...
            if "content_fts" not in tables:
                result["status"] = "missing_fts"
                return result
            fts_count = con.execute("SELECT COUNT(*) FROM content_fts").fetchone()[0]
            result["fts_entries"] = fts_count
            if "embeddings" not in tables:
                result["status"] = "vector_missing"
                return result
            vec_count = con.execute("SELECT COUNT(*) FROM embeddings").fetchone()[0]
            result["vec_entries"] = vec_count
            if fts_count > 0:
                gap_ratio = max(0.0, (fts_count - vec_count) / float(fts_count))
            else:
                gap_ratio = 0.0
            result["gap_ratio"] = round(gap_ratio, 4)
            warn_threshold = float(os.environ.get("OHM_EPI_GAP_WARN", EPI_GAP_WARN_DEFAULT))
            min_docs = int(os.environ.get("OHM_EPI_GAP_MIN_DOCS", EPI_GAP_MIN_DOCS_DEFAULT))
            if fts_count >= min_docs and gap_ratio >= warn_threshold:
                result["status"] = "warn"
            else:
                result["status"] = "ok"
        except Exception:
    ...
    def _read_pressure(self, path: str) -> dict:
        result = {}
        try:
    ...
    def _memory_status(self) -> dict:
        ...
        avail_kb = float(meminfo.get("MemAvailable", meminfo.get("MemFree", 0.0)))
        used_kb = max(0.0, total_kb - avail_kb)
        used_pct = (used_kb / total_kb * 100.0) if total_kb else 0.0

        swap_total_kb = float(meminfo.get("SwapTotal", 0.0))
        swap_free_kb = float(meminfo.get("SwapFree", 0.0))
        swap_used_kb = max(0.0, swap_total_kb - swap_free_kb)
        swap_used_pct = (swap_used_kb / swap_total_kb * 100.0) if swap_total_kb else 0.0

        psi = self._read_pressure("/proc/pressure/memory")
        psi_some = float(psi.get("some", {}).get("avg10", 0.0))
        psi_full = float(psi.get("full", {}).get("avg10", 0.0))

        warn_pct = float(os.environ.get("OHM_MEM_WARN_PCT", MEM_WARN_PCT_DEFAULT))
        crit_pct = float(os.environ.get("OHM_MEM_CRIT_PCT", MEM_CRIT_PCT_DEFAULT))
        swap_warn_pct = float(os.environ.get("OHM_SWAP_WARN_PCT", SWAP_WARN_PCT_DEFAULT))
        swap_crit_pct = float(os.environ.get("OHM_SWAP_CRIT_PCT", SWAP_CRIT_PCT_DEFAULT))
        psi_some_warn = float(os.environ.get("OHM_PSI_SOME_WARN", PSI_SOME_WARN_DEFAULT))
        psi_full_warn = float(os.environ.get("OHM_PSI_FULL_WARN", PSI_FULL_WARN_DEFAULT))
        psi_full_crit = float(os.environ.get("OHM_PSI_FULL_CRIT", PSI_FULL_CRIT_DEFAULT))

        pressure_state = "nominal"
        if used_pct >= crit_pct or swap_used_pct >= swap_crit_pct or psi_full >= psi_full_crit:
            pressure_state = "critical"
        elif (
            used_pct >= warn_pct
            or swap_used_pct >= swap_warn_pct
            or psi_some >= psi_some_warn
            or psi_full >= psi_full_warn
        ):
            pressure_state = "elevated"

        return {
            "mem_total_mb": round(total_kb / 1024.0, 1) if total_kb else 0.0,
            "mem_available_mb": round(avail_kb / 1024.0, 1) if avail_kb else 0.0,
            "mem_used_mb": round(used_kb / 1024.0, 1) if used_kb else 0.0,
            "mem_used_pct": round(used_pct, 2),
            "swap_total_mb": round(swap_total_kb / 1024.0, 1) if swap_total_kb else 0.0,
            "swap_used_mb": round(swap_used_kb / 1024.0, 1) if swap_used_kb else 0.0,
            "swap_used_pct": round(swap_used_pct, 2),
            "psi_some_avg10": round(psi_some, 4),
            "psi_full_avg10": round(psi_full, 4),
