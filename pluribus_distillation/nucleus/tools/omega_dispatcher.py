...
class ThompsonSelector:
    """
    Agent B: Evolution - Thompson Sampling for Task Prioritization.
    Uses Beta Distribution (alpha=successes, beta=failures) to select targets.
    Includes logging and bus emission for observability.
    """
    def __init__(self):
        self.stats = {}  # target -> {alpha: 1, beta: 1}
        self._selection_count = 0
        self._last_emit_time = 0

    def update(self, target: str, success: bool):
        if target not in self.stats:
            self.stats[target] = {"alpha": 1, "beta": 1}
        
        if success:
            self.stats[target]["alpha"] += 1
        else:
            self.stats[target]["beta"] += 1
        
        # Log update
        s = self.stats[target]
        win_rate = s["alpha"] / (s["alpha"] + s["beta"])
        print(f"[Thompson] {target}: {'✓' if success else '✗'} -> α={s['alpha']} β={s['beta']} (win={win_rate:.1%})")

    def get_confidence_interval(self, target: str, ci: float = 0.95) -> tuple[float, float]:
        """Compute credible interval for target's success rate."""
        from scipy import stats as scipy_stats
        s = self.stats.get(target, {"alpha": 1, "beta": 1})
        lower = scipy_stats.beta.ppf((1 - ci) / 2, s["alpha"], s["beta"])
        upper = scipy_stats.beta.ppf((1 + ci) / 2, s["alpha"], s["beta"])
        return (lower, upper)

    def get_stats_summary(self) -> dict[str, Any]:
        """Get summary of all target statistics."""
        summary = {}
        for target, s in self.stats.items():
            total = s["alpha"] + s["beta"] - 2  # Subtract prior
            win_rate = s["alpha"] / (s["alpha"] + s["beta"])
            summary[target] = {
                "alpha": s["alpha"],
                "beta": s["beta"],
    ...
    def select(self, candidates: list[tuple[str, dict[str, Any]]]) -> list[tuple[str, dict[str, Any]]]:
        """Sort candidates by sampled Beta value (descending)."""
        scored = []
        for target, state in candidates:
            # Default prior
            s = self.stats.get(target, {"alpha": 1, "beta": 1})
            # Sample from Beta
            score = random.betavariate(s["alpha"], s["beta"])
            scored.append((score, target, state))
        ...
        if scored:
            winner = scored[0]
            print(f"[Thompson] Selection #{self._selection_count}: {winner[1]} (sampled={winner[0]:.3f})")
...
def _mark_target(dispatch: dict[str, Any], target: str, *, status: str) -> None:
    ...
    # Agent B: Update Thompson stats based on outcome
    if status == "done":
        SELECTOR.update(target, success=True)
    elif status in ("failed", "access_denied"): # explicit failures
        SELECTOR.update(target, success=False)
    ...
    *,
    ...
    if RingGuard is not None:
        try:
            ...
            if task_path:
                decision = guard.check_access(target, task_path)
                # Use .allowed for observe mode support (always True in observe mode)
                # Use .granted for strict policy check
                ...
                if not access_allowed:
                    ...
                    # Log observe mode policy violation (allowed but not granted)
                    emit_bus_event(
                        bus_dir,
                        topic="omega.dispatch.access_observe",
                        kind="log",
                        level="info",
                        actor=actor,
                        data={
    ...
    while True:
        ...
        if time.time() - last_tick >= tick_s:
            ...
            for dispatch in list(state.get("dispatches", {}).values()):
                ...
                # Agent B: Use Thompson Sampling to prioritize targets
                pending_targets = _pending_targets(dispatch)
                selected_targets = SELECTOR.select(pending_targets)

                for target, tstate in selected_targets:
